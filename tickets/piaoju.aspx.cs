...
namespace tickets
{
	...
	public class piaoju : System.Web.UI.Page
	{
		...
		{    
			...
			// sqlInsertCommand1
			// 
			this.sqlInsertCommand1.CommandText = @"INSERT INTO dingpiao(车次, 出发站, 到达站, 座位类型, 票数, 总价格, 用户名, 联系方式, 地址, 身份证, 发车日期, 时间, 车厢号, 座位号, 车辆类型, 售票日期, 售票点编号, 是否学生票) VALUES (@车次, @出发站, @到达站, @座位类型, @票数, @总价格, @用户名, @联系方式, @地址, @身份证, @发车日期, @时间, @车厢号, @座位号, @车辆类型, @售票日期, @售票点编号, @是否学生票); SELECT 序号, 车次, 出发站, 到达站, 座位类型, 票数, 总价格, 用户名, 联系方式, 地址, 身份证, 发车日期, 时间, 车厢号, 座位号, 车辆类型, 售票日期, 售票点编号, 是否学生票 FROM dingpiao WHERE (序号 = @@IDENTITY)";
			...
			this.sqlInsertCommand1.Parameters.Add(new System.Data.SqlClient.SqlParameter("@是否学生票", System.Data.SqlDbType.VarChar, 10, "是否学生票"));
			// 
			// sqlUpdateCommand1
			// 
			this.sqlUpdateCommand1.CommandText = "UPDATE dingpiao SET 车次 = @车次, 出发站 = @出发站, 到达站 = @到达站, 座位类型 = @座位类型, 票数 = @票数, 总价格" +
				...
				" 车厢号 = @车厢号, 座位号 = @座位号, 车辆类型 = @车辆类型, 售票日期 = @售票日期, 售票点编号 = @售票点编号, 是否学生票 = @是否" +
				"学生票 WHERE (序号 = @Original_序号) AND (出发站 = @Original_出发站 OR @Original_出发站 IS NULL " +
				"AND 出发站 IS NULL) AND (到达站 = @Original_到达站 OR @Original_到达站 IS NULL AND 到达站 IS NU" +
				"LL) AND (发车日期 = @Original_发车日期 OR @Original_发车日期 IS NULL AND 发车日期 IS NULL) AND (" +
				"售票日期 = @Original_售票日期 OR @Original_售票日期 IS NULL AND 售票日期 IS NULL) AND (售票点编号 = @" +
				"Original_售票点编号 OR @Original_售票点编号 IS NULL AND 售票点编号 IS NULL) AND (地址 = @Original" +
				"_地址 OR @Original_地址 IS NULL AND 地址 IS NULL) AND (座位号 = @Original_座位号 OR @Origina" +
				"l_座位号 IS NULL AND 座位号 IS NULL) AND (座位类型 = @Original_座位类型 OR @Original_座位类型 IS N" +
				"ULL AND 座位类型 IS NULL) AND (总价格 = @Original_总价格 OR @Original_总价格 IS NULL AND 总价格 " +
				"IS NULL) AND (时间 = @Original_时间 OR @Original_时间 IS NULL AND 时间 IS NULL) AND (是否学" +
				"生票 = @Original_是否学生票 OR @Original_是否学生票 IS NULL AND 是否学生票 IS NULL) AND (用户名 = @O" +
				"riginal_用户名 OR @Original_用户名 IS NULL AND 用户名 IS NULL) AND (票数 = @Original_票数 OR " +
				"@Original_票数 IS NULL AND 票数 IS NULL) AND (联系方式 = @Original_联系方式 OR @Original_联系方" +
				"式 IS NULL AND 联系方式 IS NULL) AND (身份证 = @Original_身份证 OR @Original_身份证 IS NULL AN" +
				"D 身份证 IS NULL) AND (车厢号 = @Original_车厢号 OR @Original_车厢号 IS NULL AND 车厢号 IS NULL" +
				") AND (车次 = @Original_车次 OR @Original_车次 IS NULL AND 车次 IS NULL) AND (车辆类型 = @Or" +
				"iginal_车辆类型 OR @Original_车辆类型 IS NULL AND 车辆类型 IS NULL); SELECT 序号, 车次, 出发站, 到达站" +
				", 座位类型, 票数, 总价格, 用户名, 联系方式, 地址, 身份证, 发车日期, 时间, 车厢号, 座位号, 车辆类型, 售票日期, 售票点编号, 是否学生" +
				"票 FROM dingpiao WHERE (序号 = @序号)";
			...
			this.sqlUpdateCommand1.Parameters.Add(new System.Data.SqlClient.SqlParameter("@是否学生票", System.Data.SqlDbType.VarChar, 10, "是否学生票"));
			...
			this.sqlUpdateCommand1.Parameters.Add(new System.Data.SqlClient.SqlParameter("@Original_是否学生票", System.Data.SqlDbType.VarChar, 10, System.Data.ParameterDirection.Input, false, ((System.Byte)(0)), ((System.Byte)(0)), "是否学生票", System.Data.DataRowVersion.Original, null));
			...
			this.sqlDeleteCommand1.Parameters.Add(new System.Data.SqlClient.SqlParameter("@Original_是否学生票", System.Data.SqlDbType.VarChar, 10, System.Data.ParameterDirection.Input, false, ((System.Byte)(0)), ((System.Byte)(0)), "是否学生票", System.Data.DataRowVersion.Original, null));
		...
		private void xianshichepiao()
		{
		  this.sqlDataAdapter_dingpiao.SelectCommand.CommandText="select * from dingpiao ";
			...
			this.Label_cllx.Text=this.dataSet_dingp.Tables[0].Rows[count][15].ToString().Trim()+"列车";
			if(this.dataSet_dingp.Tables[0].Rows[count][18].ToString().Trim()=="是")
				this.x.Text="学";
			else this.x.Text="";
		...
		private void xianshikt()
		{
			...
			this.sqlDataAdapter_tickets.SelectCommand.CommandText="select * from  tickets where 车次='"+this.Lb_checi.Text.Trim()+"'";
			if(this.sqlConnection1.State.ToString()=="closed")
				this.sqlConnection1.Open();
